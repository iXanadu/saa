---
name: startup
description: When a new session begins in this Django project, run this to orient on project state, memory, migrations, and recent work before doing anything else.
---
...
- If absent:
  ...
  Note: for fresh templated projects with no `manage.py` yet, `/init` may also write `.engram.cfg` during scaffolding. If both run, the user's confirmed name takes precedence.
...
Note the date the handoff was written. If it's more than a day or two old, treat it as a starting point rather than a complete picture — the exhaustive search in Step 4 may surface newer state the handoff doesn't mention.
...
2. `memory_search` query="decision architecture models" scope=project limit=10 — design and data-model decisions
3. `memory_search` query="migration deployment production" scope=project limit=5 — schema and deploy state
4. `memory_search` query="wip current working feature" scope=project limit=5 — active work

### Shared + machine
5. `memory_search` scope=shared limit=5 — cross-project Django/Python lessons (use a query describing this project's domain)
6. `memory_search` scope=machine limit=3 — local env, paths, services, database
...
Watch for gaps like: handoff says "migration pending" but recent memories discuss post-migration work, or handoff predates a strategic pivot shown in recent sessions.
...
Read `.claude/CLAUDE.md` to understand the project's structure, conventions, and commands.
...
git status
git log --oneline -10
python manage.py showmigrations --plan 2>/dev/null | tail -20
```
...
- Summarize current development + deployment status (code, migrations, production)
